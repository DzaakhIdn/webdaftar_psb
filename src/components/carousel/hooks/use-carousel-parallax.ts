...
export function useParallax(mainApi?: EmblaCarouselType | null, parallax?: boolean | number) {
  ...
  const tweenParallax = useCallback((_mainApi: EmblaCarouselType, eventName?: string) => {
    ...
    _mainApi.scrollSnapList().forEach((scrollSnap, snapIndex) => {
      ...
      const slidesInSnap = (engine as any).slideRegistry[snapIndex] as number[];

      slidesInSnap.forEach((slideIndex: number) => {
        if (isScrollEvent && !slidesInView.includes(slideIndex)) return;

        if (engine.options.loop) {
          (engine as any).slideLooper.loopPoints.forEach((loopItem: any) => {
