...
function colorVars(theme: { vars: { palette: any } }, variant: string) {
  const {
    vars: { palette },
  } = theme;
...
function verticalVars(theme: { shape: any }) {
  const { shape } = theme;

  return {
    ...colorVars(theme as any, "vertical"),
    "--nav-item-gap": "4px",
...
function miniVars(theme: { shape: any }) {
  const { shape } = theme;

  return {
    ...colorVars(theme as any, "mini"),
    "--nav-item-gap": "4px",
    "--nav-item-radius": `${shape.borderRadius}px`,
    // root
...
function horizontalVars(theme: { shape: any }) {
  const { shape } = theme;

  return {
    ...colorVars(theme as any, "horizontal"),
    "--nav-item-gap": "6px",
    "--nav-height": "56px",
