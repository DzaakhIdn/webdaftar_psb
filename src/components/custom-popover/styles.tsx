...
const getRtlPosition = ({position, isRtl, value}: { position: string; isRtl: boolean; value: number }) => ({
  [position]: isRtl ? 'auto' : value,
  [position === 'left' ? 'right' : 'left']: isRtl ? value : 'auto',
});

const createBackgroundStyles = ({theme, color, size}: { theme: any; color: string; size: number }) => {
